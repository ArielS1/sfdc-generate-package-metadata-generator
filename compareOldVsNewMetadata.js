//this scripts allows to check if any directories or children are missing after generation

// this is the last manually created metadata.js
const oldMetadata = {  
   /*"installedPackages":{  
...
// replace this object with the last generated metadata object
const newMetadata = {
  "ChatterExtensions": {
...
const notExistDirectories = [];
const notExistChildren = [];
let counter = 0;
for(const key in oldMetadata) {
    console.log('checking "' + key + '", ' + newMetadata[key] + ', ' + counter);
    directory = newMetadata[key];
    if(directory) {        
        for(const child in oldMetadata[key].children) {
            if(directory.children[child]) {

            } else {
                notExistChildren.push(key + ':' + child);
            }
        }
    } else {
        counter++;
        notExistDirectories.push(key);
    }
}

console.log('notExistDirectories:', notExistDirectories);
console.log('notExistChildren:', notExistChildren);
